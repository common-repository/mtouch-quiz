<?php
	...
	if (true) {
		...
		if( $random_questions == 1 && $input_number_questions > 0 ) { // Select questions randomly
			$all_question = $wpdb->get_results($wpdb->prepare("SELECT ID,question,explanation, point_value FROM {$wpdb->prefix}mtouchquiz_question WHERE quiz_id=%d AND ID>=$first_id_value AND ID<=$last_id_value ORDER BY RAND($rand_seed) LIMIT 0, $input_number_questions", $quiz_id));
									...
									echo   "<div class='mtq_question mtq_scroll_item-$mtqid' id='mtq_question-$question_count-$mtqid'>"; 
											...
											$mtq_all_vars.=   "<input type='hidden' id='mtq_points_awarded-{$question_count}-$mtqid' value='0'/>";
											 echo   "<table class='mtq_answer_table'>";
												 ...
												 echo   "</colgroup>";
												if ( $random_answers == 1 ) {
													$dans = $wpdb->get_results("SELECT ID,answer,correct,hint FROM {$wpdb->prefix}mtouchquiz_answer WHERE question_id={$ques->ID} ORDER BY RAND($rand_seed)"); // This will randomize the question answer order
												} else {
